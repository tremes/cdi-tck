...
To execute tests within Core and EE parts of the specification you need to use in-container mode with the JBoss TCK runner, you first have to setup WildFly as described in the <<tck-in-jboss-as,Running the TCK against the CDI RI>> callout.
...
=== Running the Tests In the Container - SE parts

To execute full TCK testsuite you have to run tests within SE group as well. For this you need to execute "weld-se" Maven profile from the JBoss TCK runner POM file as follows: 
...
This profile uses link:$$https://github.com/tremes/arquillian-container-se$$[Arquillian container SE] which enables you to run SE tests with isolated configurable classpath.
