...
public class RepeatableQualifiersTest extends AbstractTest {
    ...
    @Test
    public void allProcessesAvailable() {
        Assert.assertEquals(processInstance.stream().count(), 4);
        Assert.assertNotNull(processInstance.select(Default.Literal.INSTANCE).get());
        processInstance.select(new Start.StartLiteral("A")).get();
    }
}
